...
function buildTimeline(): TimelineBlock[] {
  return [
    ...
    timelineBlock(
      ...
      300,
      'S1',
      'Mỗi em viết một điều muốn tự làm được cuối tiết. Sau 45 giây, mời 2-3 em chia sẻ; tổng hợp từ khóa lên bảng phụ.',
      'Viết một mục tiêu cá nhân ngắn và cách em sẽ chứng minh đã đạt.',
...
function buildCheckpoints(): Checkpoint[] {
  return [
    ...
    },
    {
      ...
      stepId: 'P08',
      kind: 'in_class',
      prompt: 'Điền dấu vào 15x + 10y … 150 để biểu diễn “không vượt quá 150 nghìn đồng”. Chọn một phương án, rồi giải thích trong vở.',
      responseType: 'choice',
    ...
    },
    {
      ...
      stepId: 'P20',
      kind: 'in_class',
      prompt: 'Cùng nhóm xét 3x + 2y ≤ 30: vẽ đường biên, kiểm tra (6;5) và (6;7). Mỗi em gửi kết luận về hai điểm, kèm một phép tính làm bằng chứng.',
      responseType: 'text',
...
const TV_SLIDES: Record<string, TvSlideCopy> = {
  ...
  P03: {
    ...
    action: 'Mở màn hình của em: viết một mục tiêu và một sản phẩm sẽ chứng minh em đã đạt.',
  },
  P05: {
  ...
  P20: {
    ...
    body: [
      ...
      'Nhiệm vụ: với 3x + 2y ≤ 30, vẽ đường biên và kiểm tra hai điểm (6;5), (6;7).',
      'Phân vai: một bạn vẽ, một bạn tính, một bạn hỏi “vì sao?”. Cùng đối chiếu cách làm.',
      'Sản phẩm: hình vẽ trong vở và kết luận về từng điểm, kèm phép kiểm.',
    ].join('\n'),
