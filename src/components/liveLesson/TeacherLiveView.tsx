...
import { closeLiveLessonSession, subscribeToTeacherResponses, updateLiveLessonState } from '../../services/liveLessonService';
import { LiveActivityPublisher } from './LiveActivityPublisher';
...
import { buildPreviewModel, buildPreviewZipEntries } from '../../lib/liveLesson/v4/previewBundle';
import { buildTeacherActivityRows } from '../../lib/liveLesson/teacherActivityStats';
import JSZip from 'jszip';

// Helper tải file cục bộ — inline để không kéo pdfjs (qua fileUtils) vào graph
// của component (pdfjs cần DOMMatrix, vỡ ở môi trường test node).
const previewSafeFilename = (name: string): string =>
  (name || 'bai-hoc').normalize('NFC').replace(/[^\p{L}\p{N}\-_]+/gu, '_').replace(/_+/g, '_').replace(/^_|_$/g, '').slice(0, 80) || 'bai-hoc';
...
export const getTeacherMobileControlModel = (
  ...
  return {
    ...
    secondaryLabels: {
      ...
      stats: session.publicStatsEnabled ? 'Ẩn thống kê trên TV' : 'Hiện thống kê trên TV',
      close: 'Đóng phiên',
    },
  };
};
...
export const TeacherLiveView = ({ definition, session, sessionError, onSessionChange, definitionKey }: TeacherLiveViewProps) => {
  ...
  const teacherRows = useMemo(() => responseStepId ? buildTeacherActivityRows(responses, responseStepId) : [], [responses, responseStepId]);
  const responseOptions = definition.responseSteps.find(step => step.id === responseStepId)?.options ?? [];
  const practiceSteps = useMemo(() => (cue.responseStepIds ?? []).map(stepId => definition.responseSteps.find(step => step.id === stepId)).filter((item): item is NonNullable<typeof item> => Boolean(item)), [cue.responseStepIds, definition.responseSteps]);
  ...
  const downloadPreviewBundle = useCallback(async () => {
    setPreviewBusy(true);
    try {
      const model = buildPreviewModel(definition, definitionKey ?? definition.lessonId);
      let posterDataUri: string | null = null;
      const posterCue = model.cues.find(item => item.media);
      if (posterCue?.media) {
        try {
          const response = await fetch(posterCue.media.poster);
          const blob = await response.blob();
          posterDataUri = await new Promise<string>((resolve, reject) => {
            const reader = new FileReader();
            reader.onload = () => resolve(String(reader.result));
            reader.onerror = () => reject(reader.error);
            reader.readAsDataURL(blob);
          });
        } catch { posterDataUri = null; }
      }
      const zip = new JSZip();
      for (const entry of buildPreviewZipEntries(model, posterDataUri, v4Contract)) zip.file(entry.name, entry.content);
      const archive = await zip.generateAsync({ type: 'blob' });
      triggerDownload(archive, `${previewSafeFilename(definition.title)}_xem-truoc-TV-HS.zip`);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Không tạo được bộ xem trước.');
    } finally {
      setPreviewBusy(false);
    }
  }, [definition, definitionKey]);

  return <main className="min-h-screen overflow-x-hidden bg-slate-100 pb-28 text-slate-900 sm:pb-32">{responseStepId && <LiveActivityPublisher key={`${session.id}:${cue.id}`} sessionId={session.id} cueId={cue.id} stepId={responseStepId} enabled={session.publicStatsEnabled && session.status !== 'closed'} />}<div className="mx-auto w-full max-w-6xl min-w-0 space-y-3 px-3 py-3 sm:space-y-4 sm:p-6">
    ...
    <div className="flex flex-wrap gap-2"><button type="button" onClick={() => setShowOfflineChecklist(!showOfflineChecklist)} className="rounded-xl border border-amber-300 bg-amber-50 px-4 py-2 text-xs font-black text-amber-800 hover:bg-amber-100">{showOfflineChecklist ? 'Ẩn gói offline' : 'Xem gói offline trước tiết'}</button><button type="button" onClick={() => void downloadPreviewBundle()} disabled={previewBusy} className="rounded-xl border border-cyan-300 bg-cyan-50 px-4 py-2 text-xs font-black text-cyan-800 hover:bg-cyan-100 disabled:opacity-50">{previewBusy ? 'Đang tạo bộ xem trước…' : 'Tải bộ xem trước (TV + HS + GV)'}</button></div>
    ...
    {responseStepId && !isClosed && <section className="rounded-2xl border border-cyan-200 bg-cyan-50 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-bold text-cyan-950">Kết quả của hoạt động đang dạy</p><p className="mt-1 text-sm text-cyan-800">{session.publicStatsEnabled ? 'Đã bật hiển thị. Phản hồi mới cập nhật lên TV theo hoạt động.' : 'Đang ẩn để học sinh tự suy nghĩ. Mở khi cần đối chiếu và thảo luận.'}</p></div><button type="button" aria-pressed={session.publicStatsEnabled} onClick={() => void applyPatch({ publicStatsEnabled: !session.publicStatsEnabled })} className="min-h-12 rounded-xl bg-cyan-900 px-5 py-3 font-bold text-white">{session.publicStatsEnabled ? 'Ẩn kết quả trên TV' : 'Hiện kết quả lớp'}</button></div>{responseStepId === 'cp-group-product' && <p className="mt-3 text-sm text-cyan-900">Thông báo số nhóm từ 1–12. HS chọn số nhóm trên máy rồi mỗi em gửi phần giải thích của mình; TV hiển thị tiến độ theo nhóm đã chọn.</p>}</section>}
    ...
    {responseStepId && <section className="rounded-2xl border border-indigo-100 bg-white p-4 shadow-sm" aria-label="Bằng chứng riêng theo học sinh"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-widest text-indigo-600">Chỉ giáo viên</p><h2 className="mt-1 text-lg font-black text-slate-900">Bằng chứng theo từng học sinh</h2><p className="mt-1 text-xs font-semibold leading-5 text-slate-500">Nhãn HS chỉ có ý nghĩa trong phiên này. Lượt gửi không tự đồng nghĩa với đạt.</p></div>{privateNeedsSummary.lines.length > 0 && <div className="rounded-xl bg-amber-50 px-3 py-2 text-xs font-bold text-amber-900">{privateNeedsSummary.lines.join(' · ')}</div>}</div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[560px] text-left text-sm"><thead className="border-b border-slate-100 text-xs uppercase tracking-wider text-slate-500"><tr><th className="px-3 py-2">HS</th><th className="px-3 py-2">Trạng thái</th><th className="px-3 py-2">Đánh giá</th><th className="px-3 py-2">Lựa chọn</th></tr></thead><tbody>{teacherRows.length === 0 ? <tr><td colSpan={4} className="px-3 py-5 text-center font-semibold text-slate-500">Chưa có phản hồi ở hoạt động này.</td></tr> : teacherRows.map(row => <tr key={row.participantUid} className="border-b border-slate-50"><td className="px-3 py-2 font-black text-slate-800">{row.label}</td><td className="px-3 py-2 font-semibold text-emerald-700">{row.submitted ? 'Đã gửi' : 'Chưa gửi'}</td><td className={`px-3 py-2 font-black ${row.assessment === 'Đúng' ? 'text-emerald-700' : row.assessment === 'Sai' ? 'text-rose-700' : 'text-amber-700'}`}>{row.assessment}</td><td className="px-3 py-2 font-semibold text-slate-600">{row.selectedValue ? responseOptions.find(option => option.value === row.selectedValue)?.label ?? row.selectedValue : '—'}</td></tr>)}</tbody></table></div></section>}
    {practiceSteps.length > 1 && <section className="rounded-2xl border border-violet-100 bg-violet-50 p-4 shadow-sm" aria-label="Dashboard luyện tập theo bài"><div><p className="text-xs font-black uppercase tracking-widest text-violet-700">V7.2 · Luyện tập tách từng bài</p><h2 className="mt-1 text-lg font-black text-violet-950">A · B · C · D · Challenge</h2><p className="mt-1 text-xs font-semibold text-violet-800">GV xem riêng từng question ID; không gộp thành một lượt gửi chung.</p></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{practiceSteps.map((practiceStep, index) => { const rows = buildTeacherActivityRows(responses, practiceStep.id); const correct = rows.filter(row => row.assessment === 'Đúng').length; const review = rows.filter(row => row.assessment === 'Cần GV xem').length; return <article key={practiceStep.id} className="rounded-xl border border-violet-200 bg-white p-3"><p className="text-sm font-black text-violet-900">{String.fromCharCode(65 + Math.min(index, 3))}{index === 4 ? ' · Challenge' : ''}</p><p className="mt-2 text-2xl font-black tabular-nums text-violet-700">{rows.length}</p><p className="text-xs font-semibold text-slate-500">đã gửi</p><p className="mt-2 text-xs font-bold text-emerald-700">{correct} đúng</p>{review > 0 && <p className="text-xs font-bold text-amber-700">{review} cần xem</p>}</article>; })}</div></section>}
