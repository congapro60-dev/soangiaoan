...
export const StudentLiveView = ({ definition, sessionId, expectedClassId, expectedJoinCode, publicState, publicStateError = null }: StudentLiveViewProps) => {
  ...
  // P31 currently publishes approved glossary translations only; its screens,
  // cues and tasks do not have a complete reviewed localized content pack.
  const fullTranslationAvailable = languageView.language !== 'vi'
    && hasApprovedFullTranslationPack(languageView.language, v4Contract.glossary, false);
  ...
  const makeRuntimeLanguageView = useCallback((view: StudentLanguageView): StudentLanguageView => {
    ...
    if (view.language !== 'vi' && view.supportMode === 'approved_full_translation') {
      const available = hasApprovedFullTranslationPack(view.language, v4Contract.glossary, false);
  ...
  const pickLanguage = (language: V4Language, requestedMode: SupportMode = language === 'vi' ? 'vi_anchor' : 'bilingual') => {
    ...
    const selectedFullTranslationAvailable = language !== 'vi'
      && hasApprovedFullTranslationPack(language, v4Contract.glossary, false);
