...
import { isWeek5SourceKey, WEEK5_SOURCE_KEYS } from './week5Catalog';

describe('Week 5 V7.2 source catalog', () => {
  it('contains exactly eight packages per grade', () => {
    expect(WEEK5_SOURCE_KEYS).toHaveLength(24);
    expect(WEEK5_SOURCE_KEYS.filter(key => key.startsWith('10-'))).toHaveLength(8);
    expect(WEEK5_SOURCE_KEYS.filter(key => key.startsWith('11-'))).toHaveLength(8);
    expect(WEEK5_SOURCE_KEYS.filter(key => key.startsWith('12-'))).toHaveLength(8);
  });
  ...
  it('maps every source key to a V7.2 14-activity contract with five practice questions', () => {
    for (const sourceKey of WEEK5_SOURCE_KEYS) {
      ...
      expect(isWeek5SourceKey(contract.sourceKey ?? '')).toBe(true);
