...
**Tải PDF bản phụ huynh** (nối tiếp): nút cũ gọi `window.print()` nên in cả app (giống ảnh chụp màn hình); bản iframe/`window.open` từng ra tab trắng. Chốt: dùng ĐÚNG cơ chế xuất PDF của giáo án — `parentReportPrintDoc.ts::exportParentReportToPdf` dựng báo cáo vào node ẩn ngoài màn hình (style scope theo `#parent-report-pdf-root`, header HS/lớp/mã/ngày, 4 số liệu, nhận xét chung, điểm mạnh/cần rèn, bảng kết quả, 2 cột đồng hành, ô ký PH+GV — chỉ từ `ParentSafeReport`, escape HTML) rồi gọi `utils/pdfExport.ts::exportElementToPdf` (html2canvas-pro + jsPDF → `pdf.save()`). **Tải thẳng file .pdf**, không tab/không hộp thoại in. Nghiệm thu: parentReportPrintDoc 3 test; smoke live dev-server: render 780×1030px, xuất blob application/pdf ~278KB, no error.
