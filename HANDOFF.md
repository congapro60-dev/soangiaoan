...
`main` hiện ở `7c80579`, gồm nền tảng các lô trước và lô classroom grading mới nhất:
...
- Full Vitest: **82 files / 1.129 tests pass**.
...
- `git diff --check`: pass.
- Ox Alpha Free/OpenCode audit: model `opencode/x-preview-f-free` — **PASS 7/7 hạng mục**.
- Production smoke: đã mở production và đọc lại lớp `11Columbus`/Bài nộp, thấy dữ liệu thật và nhiều lượt nộp; chỉ bấm làm mới, không tạo/sửa/xóa/chấm dữ liệu.
...
- Sau khi deploy cần xác nhận Vercel deployment của commit `7c80579` ở trạng thái Ready/Production trước khi dùng nút mới trên dữ liệu thật.
...
1. Xác nhận deployment Vercel commit `7c80579` là **Ready / Production**; không chạy thao tác dữ liệu thật trước bước này.
