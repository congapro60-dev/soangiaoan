...
- **PDF là ẢNH CHỤP giao diện web, không sinh từ file Word.** `exportToPDF` clone `.wmde-markdown` rồi gọi `window.print()`; Word form Toán thì dựng lại từ `ToanLessonModel`. Hai đường không bao giờ giống hệt nhau (font, khổ, công thức OMML vs KaTeX). Muốn giống thật phải chuyển .docx → PDF phía server — việc lớn, chưa làm.
